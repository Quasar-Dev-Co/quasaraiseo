  ...
  const sidebarContent = (
    <aside className="relative overflow-hidden flex h-full w-[260px] flex-col border-r border-slate-200/80 bg-white/80 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/80">
      ...
      <div className="flex items-center gap-3 px-5 pt-6 pb-7">
        <span className="grid size-9.5 place-items-center overflow-hidden rounded-[13px] border border-blue-500/30 bg-slate-950 p-1.5 shadow-[0_8px_20px_rgba(59,130,246,0.25)]">
        ...
        <div>
          <div className="text-sm font-extrabold tracking-[0.08em] text-slate-950 dark:text-white uppercase leading-none">
            Quasar<span className="bg-gradient-to-r from-blue-600 via-sky-500 to-cyan-400 bg-clip-text text-transparent">AISEO</span>
          </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active
                  ? "bg-gradient-to-r from-blue-500/15 via-sky-500/10 to-transparent text-blue-950 dark:text-blue-300 font-bold"
                  : "text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-white"
              )}
            >
              {active && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 h-[24px] w-1 rounded-r-full bg-gradient-to-b from-blue-600 via-sky-500 to-cyan-400 shadow-[0_0_12px_rgba(59,130,246,0.5)]" />
              )}
              <Icon className={cn("size-[18px] shrink-0", active ? "text-blue-600 dark:text-blue-400" : "")} />
      ...
      <nav className="flex flex-col gap-1 px-3">
        {googleItems.map((item) => {
          ...
          return (
            ...
            >
              {active && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 h-[22px] w-1 rounded-r-full bg-gradient-to-b from-blue-600 via-sky-500 to-cyan-400 shadow-[0_0_12px_rgba(59,130,246,0.5)]" />
              )}
              <Icon className={cn("size-[17px] shrink-0", active ? "text-blue-600 dark:text-blue-400" : "")} />
              <span>{item.label}</span>
              {active && <ChevronRight className="ml-auto size-3.5 text-blue-500" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="mt-auto">
        ...
        <div className="mx-3 mb-4 rounded-2xl border border-slate-200/90 bg-white/65 p-4 dark:border-white/10 dark:bg-slate-900/50">
          ...
          <div className="mt-2.5 h-[7px] w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
            <div className="h-full rounded-full bg-gradient-to-r from-blue-600 via-sky-500 to-cyan-400" style={{ width: "72%" }} />
          </div>
          <div className="mt-2 text-[10px] font-semibold text-slate-400 dark:text-slate-500">Renews in 14 days</div>
          <Button variant="outline" size="sm" className="mt-3 w-full text-xs font-bold hover:bg-blue-50 hover:text-blue-700 dark:hover:bg-blue-400/10">
            Upgrade plan
          </Button>
        </div>
        ...
        <div className="mx-3 mb-3 rounded-2xl border border-slate-200/90 bg-white/65 p-3 dark:border-white/10 dark:bg-slate-900/50">
          <div className="flex items-center gap-2.5">
            <span className="grid size-8 shrink-0 place-items-center rounded-full bg-gradient-to-br from-blue-500 via-sky-500 to-cyan-400 text-xs font-bold text-white shadow-sm">
