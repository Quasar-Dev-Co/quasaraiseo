  ...
  return (
    <section className="grid grid-cols-1 items-start gap-5.5">
      ...
      >
        ...
        <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.19em] text-emerald-300">
        ...
        <form onSubmit={handleSubmit} className="mt-8">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <label className="sm:col-span-2">
              <span className="flex justify-between text-[13px] font-bold text-slate-200">
                Audit focus <em className="text-[11px] not-italic uppercase text-emerald-300">Optional</em>
              </span>
              <textarea
                name="auditFocus"
                placeholder="Describe specific goals, priority services, target keywords, competitors, or pages that need extra attention..."
                className="mt-2.25 min-h-32 w-full resize-y rounded-[14px] border border-slate-400/16 bg-slate-900/72 px-4 py-3.75 text-sm text-white outline-none placeholder:text-slate-500 focus:border-emerald-400/65 focus:ring-4 focus:ring-emerald-500/10"
              />
              ...
              return (
                <button
                  ...
                  className={`flex min-h-9.5 items-center gap-2 rounded-full border px-3.25 text-xs font-bold transition-colors ${
                    active
                      ? "border-emerald-400/32 bg-emerald-500/10 text-emerald-300"
          ...
          <div className="mt-7 flex flex-col items-start justify-between gap-4.5 border-t border-slate-400/13 pt-6 sm:flex-row sm:items-center">
            ...
            <button
              type="submit"
              disabled={isBusy}
              className="flex min-h-13 min-w-[210px] items-center justify-center gap-2.5 rounded-[15px] bg-gradient-to-br from-emerald-300 to-emerald-500 text-sm font-bold text-[#022c22] transition-transform hover:-translate-y-px disabled:opacity-70"
            >
...
function FormField({
  ...
  return (
    <label>
      <span className="flex justify-between text-[13px] font-bold text-slate-200">
        {label} <em className="text-[11px] not-italic uppercase text-emerald-300">Required</em>
      </span>
      <div className="relative mt-2.25">
        ...
        <input
          ...
          required
          className="h-13 w-full rounded-[14px] border border-slate-400/16 bg-slate-900/72 px-11 text-sm text-white outline-none placeholder:text-slate-500 focus:border-emerald-400/65 focus:ring-4 focus:ring-emerald-500/10"
        />
      </div>
    ...
    <label>
      ...
      <div className="relative mt-2.25">
        ...
        <select
          ...
          required
          defaultValue=""
          className="h-13 w-full appearance-none rounded-[14px] border border-slate-400/16 bg-slate-900/72 px-11 text-sm text-white outline-none focus:border-emerald-400/65 focus:ring-4 focus:ring-emerald-500/10"
        >
