  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm">
      <div className="relative w-[min(520px,calc(100%-32px))] overflow-hidden rounded-[28px] border border-slate-700/50 bg-gradient-to-br from-slate-900 via-slate-950 to-black p-8 shadow-[0_40px_120px_rgba(0,0,0,0.5)]">
        ...
        <div className="pointer-events-none absolute -bottom-20 -left-20 size-60 rounded-full bg-amber-500/8 blur-3xl" />
          ...
          <div className={`grid size-12 place-items-center rounded-[16px] ${
            ...
            {isError ? (
              ...
              <div className="relative">
                ...
                <span className="absolute -inset-1 animate-ping rounded-full bg-emerald-400/20" />
              </div>
            )}
          </div>
          <div>
        ...
        {!isError && (
          <div className="relative mt-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[11px] font-bold text-emerald-300">{progress}%</span>
            </div>
        ...
        {!isError && !isComplete && activeStage && (
          <div className="relative mt-5 flex items-start gap-3 rounded-[16px] border border-emerald-400/15 bg-emerald-500/5 p-4">
            <div className="relative mt-0.5 shrink-0">
              <activeStage.icon className="size-5 text-emerald-300" />
              <span className="absolute -inset-1 animate-ping rounded-full bg-emerald-400/20" />
            </div>
            <div>
              <p className="text-sm font-semibold text-emerald-200">{activeStage.label}</p>
            ...
            {STAGES.map((stage, i) => {
              ...
              return (
                ...
                >
                  ...
                  {done && (
                    <span className="ml-auto text-[10px] font-bold uppercase tracking-wider text-emerald-500/60">
                      Done
                    </span>
                  )}
                  {active && (
                    <span className="ml-auto flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-emerald-400">
                      <span className="flex gap-0.5">
                        <span className="size-1 animate-bounce rounded-full bg-emerald-400 [animation-delay:0ms]" />
                        <span className="size-1 animate-bounce rounded-full bg-emerald-400 [animation-delay:150ms]" />
                        <span className="size-1 animate-bounce rounded-full bg-emerald-400 [animation-delay:300ms]" />
                      </span>
                    </span>
                  )}
                </div>
        ...
        {isComplete && (
          <div className="relative mt-6 flex items-center justify-center gap-2 text-[11px] text-emerald-400">
