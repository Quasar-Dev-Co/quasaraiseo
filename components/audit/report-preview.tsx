...
const scoreStyles: Record<string, { bar: string; grade: string }> = {
  "": { bar: "bg-blue-500", grade: "text-blue-600 dark:text-blue-400" },
  overall: { bar: "bg-gradient-to-r from-blue-600 to-purple-600", grade: "text-blue-300" },
...
const severityConfig: Record<string, { label: string; cls: string; dot: string }> = {
  ...
  pass: { label: "Passed", cls: "bg-blue-50 text-blue-600 dark:bg-blue-400/15 dark:text-blue-400", dot: "bg-blue-500" },
}
...
const statusConfig: Record<string, string> = {
  good: "bg-blue-50 text-blue-700 dark:bg-blue-400/15 dark:text-blue-400",
        ...
        return (
          <div key={i} className="grid grid-cols-1 gap-3 border-b border-slate-100 py-4 last:border-0 md:grid-cols-[auto_1fr_auto] dark:border-white/5">
            ...
            <div className="md:max-w-[200px]">
              ...
              <p className="text-[12px] leading-relaxed text-blue-700 dark:text-blue-400 break-words">{f.action}</p>
            </div>
          </div>
        )
      })}
    ...
    return (
      <section id="report-preview" className="mt-18">
        <div className="mb-6 flex flex-col items-start justify-between gap-7 lg:flex-row lg:items-end">
          <div>
            <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-blue-700 dark:text-blue-400">
        ...
        <div className="rounded-[30px] border border-slate-200 bg-white/80 p-12 text-center shadow-[0_28px_80px_rgba(15,23,42,0.11)] dark:border-white/10 dark:bg-slate-900/60">
          <span className="mx-auto grid size-20 place-items-center rounded-[22px] bg-blue-50 text-blue-600 dark:bg-blue-400/10 dark:text-blue-400">
    ...
    <section id="report-preview" className="mt-18">
      <div className="mb-6 flex flex-col items-start justify-between gap-7 lg:flex-row lg:items-end">
        ...
        <span className={`flex items-center gap-2 rounded-full border px-3 py-2.25 text-xs font-bold ${
          hasRealData
            ? "border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-400/20 dark:bg-blue-400/10 dark:text-blue-400"
      ...
      <div className="rounded-[30px] border border-slate-200 bg-white/80 p-5 shadow-[0_28px_80px_rgba(15,23,42,0.11)] dark:border-white/10 dark:bg-slate-900/60">
        ...
        <div className="hidden print:block mb-8 border-b-2 border-blue-500 pb-4">
        ...
        <div className="flex items-center justify-between gap-4.5 px-1.25 pt-2.25 pb-5.25">
          ...
          {isRunning ? (
            ...
            <span className="flex items-center gap-1.75 rounded-full bg-blue-50 px-2.5 py-2 text-[11px] font-bold text-blue-700 dark:bg-blue-400/15 dark:text-blue-400">
                  ...
                  {richReport.contentEeat.eeatMatrix.map((row) => (
                    <tr key={row.factor}>
                      ...
                      <td className="border-b border-slate-100 px-5 py-4 dark:border-white/5">
                        <span className={`text-lg font-black ${row.score >= 70 ? "text-blue-600 dark:text-blue-400" : row.score >= 50 ? "text-amber-600 dark:text-amber-400" : "text-red-600 dark:text-red-400"}`}>{row.score}</span>
                    ...
                    <tr key={i}>
                      ...
                      <td className="border-b border-slate-100 px-5 py-4 dark:border-white/5">
                        <b className={`rounded-full px-2.25 py-1.5 text-[10px] font-bold capitalize ${
                          kw.difficulty === "low" ? "bg-blue-50 text-blue-700 dark:bg-blue-400/15 dark:text-blue-400"
                      ...
                      <td className="border-b border-slate-100 px-5 py-4 dark:border-white/5">
                        <b className={`rounded-full px-2.25 py-1.5 text-[10px] font-bold capitalize ${
                          ...
                          : kw.priority === "quick win" ? "bg-blue-50 text-blue-700 dark:bg-blue-400/15 dark:text-blue-400"
                ...
                <tbody>
                  {richReport.backlinkProfile.topReferringDomains.map((d, i) => (
                    <tr key={i}>
                      ...
                      <td className="border-b border-slate-100 px-5 py-4 dark:border-white/5">
                        <b className={`rounded-full px-2.25 py-1.5 text-[10px] font-bold capitalize ${
                          d.assessment === "healthy" ? "bg-blue-50 text-blue-700 dark:bg-blue-400/15 dark:text-blue-400"
                    ...
                    <tr key={i}>
                      ...
                      <td className="border-b border-slate-100 px-5 py-4 dark:border-white/5">
                        <b className={`rounded-full px-2.25 py-1.5 text-[10px] font-bold ${
                          c.status === "Allowed" ? "bg-blue-50 text-blue-700 dark:bg-blue-400/15 dark:text-blue-400"
                  ...
                  <tr key={row.category}>
                    ...
                    <td className="border-b border-slate-100 px-5 py-4 dark:border-white/5">
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-black ${row.score >= 70 ? "text-blue-600 dark:text-blue-400" : row.score >= 50 ? "text-amber-600 dark:text-amber-400" : "text-red-600 dark:text-red-400"}`}>{row.score}</span>
                        <i className="h-1.25 w-16 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                          <b className="block h-full rounded-full bg-blue-500" style={{ width: `${row.score}%` }} />
                        </i>
                      </div>
                    </td>
                    <td className="border-b border-slate-100 px-5 py-4 dark:border-white/5">
                      <b className={`rounded-full px-2.25 py-1.5 text-[10px] font-bold ${
                        row.status === "Good" ? "bg-blue-50 text-blue-700 dark:bg-blue-400/15 dark:text-blue-400"
                ...
                {activeKeywords.length > 0 ? (
                  activeKeywords.map((kw) => (
                    <tr key={kw.id}>
                      ...
                      <td className="border-b border-slate-100 px-5 py-4 text-xs font-bold text-blue-600 dark:border-white/5 dark:text-blue-400">
                ...
                <div key={phaseKey} className={`mb-5 border-l-4 ${phaseColors[pIdx]} pl-5 last:mb-0`}>
                  <div className="mb-3 flex items-center gap-2.5">
                    <span className={`grid size-8 place-items-center rounded-[10px] text-xs font-black ${
                      ...
                      : "bg-blue-50 text-blue-600 dark:bg-blue-400/15 dark:text-blue-400"
                    }`}>{pIdx + 1}</span>
                ...
                >
                  <span
                    className={`grid size-10.5 place-items-center rounded-[14px] text-[13px] font-black ${
                      index === 0 ? "bg-blue-100 dark:bg-blue-400/15 dark:text-blue-400" : "bg-slate-100 dark:bg-slate-800 dark:text-slate-300"
                    }`}
                  >
                  ...
                  <aside className="flex gap-1.75">
                    <b className="rounded-full bg-blue-50 px-2.25 py-1.5 text-[10px] font-bold text-blue-700 dark:bg-blue-400/15 dark:text-blue-400">
          ...
          <div className="px-5 pb-5 pt-3.5">
            {richReport.quickWins.map((qw, i) => (
              <div key={i} className="grid grid-cols-1 items-center gap-3 border-b border-slate-100 py-3.5 last:border-0 md:grid-cols-[1fr_auto_auto] dark:border-white/5">
                ...
                <b className="w-max rounded-full bg-blue-50 px-2.25 py-1.5 text-[10px] font-bold text-blue-700 dark:bg-blue-400/15 dark:text-blue-400">{qw.impact}</b>
              </div>
            ))}
          </div>
        </ReportCard>
        ...
        {hasRealData && (
          ...
          <div className="mt-4.5 flex flex-col items-start justify-between gap-7 rounded-3xl bg-gradient-to-br from-slate-900 via-purple-950/50 to-slate-950 p-7 text-white lg:flex-row lg:items-center">
            ...
            <div className="flex gap-2.5">
              <button
                onClick={downloadPdf}
                className="flex h-11 items-center gap-2 rounded-[12px] bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 px-4 text-xs font-bold text-white shadow-[0_8px_20px_rgba(217,70,239,0.3)] transition-transform hover:-translate-y-px"
              >
