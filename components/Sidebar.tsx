  ...
  const sidebarContent = (
    <aside className="flex h-full w-[238px] flex-col border-r border-slate-200/90 bg-white/76 px-4 py-7 backdrop-blur-2xl dark:border-white/10 dark:bg-slate-900/80">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive
                  ? "text-emerald-800 bg-gradient-to-r from-emerald-400/15 to-emerald-400/5 dark:text-emerald-300 dark:from-emerald-400/20 dark:to-emerald-400/5"
                  : "text-slate-600 hover:text-emerald-700 hover:bg-emerald-400/5 dark:text-slate-400 dark:hover:text-emerald-300"
              )}
            >
              {isActive && (
                <motion.div
                  layoutId="activeTabIndicator"
                  className="absolute -left-4 top-[9px] h-[30px] w-1.5 rounded-r-lg bg-emerald-400 shadow-[0_0_14px_rgba(40,217,177,0.7)]"
                  transition={{ type: "spring", stiffness: 380, damping: 30 }}
                />
              )}
              <Icon className={cn("size-[18px] shrink-0", isActive ? "text-emerald-700 dark:text-emerald-300" : "")} />
              <span>{item.name}</span>
            </button>
          );
        })}
      ...
      <section className="mt-8 rounded-2xl border border-slate-200/90 bg-white/65 p-4.5 shadow-sm dark:border-white/10 dark:bg-slate-950/50">
        ...
        <div className="mt-2.5 h-[7px] w-full overflow-hidden rounded-full bg-emerald-50 dark:bg-slate-900">
          <motion.div
        ...
        <a
          href="#"
          className="mt-4.5 inline-flex items-center gap-2 text-xs font-extrabold text-emerald-700 hover:text-emerald-600 dark:text-emerald-400 dark:hover:text-emerald-300"
        >
          Upgrade plan
